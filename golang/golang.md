...
## 34、golang 中内存分配规格是什么

---

### Go 内存分配的核心理念

Go 的内存分配器是其并发性能和垃圾回收（GC）效率的关键所在。它的设计目标是：**快速、高效、低延迟，并能有效控制内存碎片**。其实现深受 Google 自家的 **TCMalloc (Thread-Caching Malloc)** 影响，但针对 Go 的并发模型（Goroutines）和垃圾回收机制进行了深度定制。

核心思想是将内存预先分割成不同大小的块（也就是 **Size Class**），并将这些块分级管理，从而将大部分内存分配请求转化为简单的链表操作，避免了频繁地向操作系统申请内存（这是一个昂贵的操作）和加锁竞争。

### 一、内存分配规格 (Size Class)

在 Go 中，**内存分配规格（Size Class）** 是一组预定义的、固定大小的内存块尺寸。当程序需要分配一块小内存时（小于等于 32KB），Go 不会精确地分配所请求的大小，而是会找到能容纳该请求的**最小**的 Size Class，并将该规格的整个内存块分配出去。

#### 1. Size Class 的具体规格

Go 定义了大约 **67** 个 Size Class，覆盖了从 **8 字节到 32KB** 的范围。这些规格并非线性增长，其特点是：

*   **小对象密集**：在小尺寸范围内（例如 512 字节以下），Size Class 的间隔非常小，通常是 8 或 16 字节。例如：8, 16, 24, 32, 48, 64...
*   **大对象稀疏**：随着尺寸增大，Size Class 之间的间隔会越来越大。例如，从 2KB 到 32KB 可能只有几个规格。

您可以在 Go 语言的源码 `src/runtime/sizeclasses.go` 文件中找到确切的定义。这个文件通常是自动生成的，但反映了当前的规格表。

**示例（非详尽列表，仅为说明结构）：**

| Class Index | Size (Bytes) | Objects per Page |
| :---------- | :----------- | :--------------- |
| 1           | 8            | 1024             |
| 2           | 16           | 512              |
| 3           | 24           | 341              |
...
使用这种机制的主要好处是**管理简单高效，并能有效控制内存碎片**。

*   **减少外部碎片 (External Fragmentation)**：
    *   **问题**：如果不使用规格化管理，内存中会产生大量大小不一的“空洞”。即使总空闲内存足够，也可能因为没有一块连续的大内存而导致分配失败。
    *   **解决方案**：Go 将内存页（Page，8KB）预先切分成**相同大小**的块（Objects/Spans）。当一个 Size Class 为 16 字节的块被释放后，它可以被无缝地用于下一次 9 到 16 字节的任何内存请求。这使得内存可以被高效地复用。

*   **减少内部碎片 (Internal Fragmentation)**：
    *   **代价**：这种做法会带来一定的内部碎片。例如，当您请求 9 字节内存时，Go 会分配一个 16 字节的块，其中有 7 字节是浪费的。
    *   **权衡**：通过精心设计的 Size Class 间隔（小尺寸密集），Go 将平均内部碎片率控制在一个可接受的范围（通常低于 15%）。这种可控的、少量的内部浪费，换来了分配速度的大幅提升和外部碎片的有效避免，是完全值得的。

*   **加速分配和回收**：
    *   每个 Size Class 都有自己的空闲链表。分配内存时，只需从对应链表上取下一个块；释放时，只需将块重新放回链表。这些操作非常快速，通常不需要加锁。

### 二、Go 内存分配的详细过程

Go 的内存管理模型是一个三层结构：`mcache` -> `mcentral` -> `mheap`。这个结构巧妙地平衡了并发性能和内存利用率。
...
*   **`mcache` (Memory Cache)**：
    *   **归属**：每个处理器（P）都有一个私有的 `mcache`。P 是 Go 调度器中的概念，通常数量等于 `GOMAXPROCS`。
    *   **特点**：**无锁访问**。因为 `mcache` 是 P 私有的，正在该 P 上运行的 Goroutine 在申请内存时，可以直接访问自己的 `mcache`，无需加锁，这是 Go 内存分配如此快速的核心原因。
    *   **内容**：`mcache` 内部为每个 Size Class 维护了一个空闲内存块（`mspan`）的链表。

*   **`mcentral` (Central Memory)**：
    *   **归属**：所有 P 共享的中心缓存。
    *   **特点**：**需要加锁**。当某个 P 的 `mcache` 中缺少某个 Size Class 的空闲块时，它会向 `mcentral` 申请。此操作需要加锁，以避免多个 P 同时操作 `mcentral` 产生竞态。
    *   **内容**：`mcentral` 为每个 Size Class 维护了两个 `mspan` 链表：一个链表包含非空闲块（`non-empty`），另一个链表包含已完全回收的块（`empty`）。

*   **`mheap` (Heap Area)**：
    *   **归属**：全局唯一，代表了 Go 程序向操作系统申请到的所有堆内存。
    *   **特点**：**需要加锁**。当 `mcentral` 中也缺少内存时，它会向 `mheap` 申请。`mheap` 负责管理大块内存（Page），并按需切割成 `mspan`。如果 `mheap` 自身内存也不足，它将通过系统调用（如 `mmap`）向操作系统申请新的内存。
    *   **内容**：`mheap` 将申请来的大块内存组织成页（Page, 8KB），再将连续的页组合成 `mspan`。它还维护着一个 `free` 列表，记录着不同大小（1-128页）的空闲 `mspan`。

#### 2. 内存分配流程（以小对象为例）

假设一个 Goroutine 需要分配一块 **20 字节** 的内存：

1.  **确定 Size Class**：
    *   运行时计算出 20 字节需要向上取整到哪个 Size Class。假设它落入了 **24 字节**的规格（Class Index 3）。

2.  **尝试从 `mcache` 分配**：
    *   当前 Goroutine 所在的 P 会查找其私有的 `mcache`。
    *   它会检查 `mcache` 中用于 24 字节 Size Class 的链表是否有空闲块。
    *   **如果找到**：直接从链表中取出一个块，返回给 Goroutine。这个过程**无锁**，极快。分配完成。

3.  **`mcache` 缺货，向 `mcentral` 补充**：
    *   **如果 `mcache` 中没有** 24 字节的空闲块，它会向 `mcentral` 发起请求。
    *   `mcentral` 会**加锁**，然后在其 24 字节 Size Class 的 `non-empty` 链表中查找一个有空闲空间的 `mspan`。
    *   找到后，`mcentral` 会从这个 `mspan` 中“借”一批（而不是一个）空闲块给 `mcache`，以备后续使用。这种批处理操作减少了 `mcache` 和 `mcentral` 的交互频率。
    *   `mcache` 获得补充后，再将其中一个块分配给 Goroutine。

4.  **`mcentral` 缺货，向 `mheap` 申请**：
    *   **如果 `mcentral` 的 `non-empty` 和 `empty` 链表都为空**，说明没有可用的 `mspan` 了。
    *   `mcentral` 会向 `mheap` 申请一个或多个内存页（Page）。
    *   `mheap` 会**加锁**，在其 `free` 列表中查找合适大小的连续页。
    *   **如果 `mheap` 有**：它会将这些页分配给 `mcentral`，并将其格式化为 24 字节规格的 `mspan`（即把 8KB 的页切分成约 341 个 24 字节的块）。然后 `mcentral` 再将这个新的 `mspan` 加入自己的链表，并分配给 `mcache`。
    *   **如果 `mheap` 也没有**：`mheap` 将通过系统调用向操作系统申请一块新的大内存（通常是 MB 级别）。申请成功后，`mheap` 再重复上述步骤。

#### 3. 大对象（> 32KB）和巨对象（> 16MB）的分配

*   **大对象 (> 32KB)**：
    *   这类对象会**绕过 `mcache` 和 `mcentral`**，直接由 `mheap` 进行分配。
    *   `mheap` 会在其 `free` 列表中查找能容纳该对象的最小的连续页块。例如，一个 40KB 的对象需要 5 个 8KB 的页。
    *   这种分配同样需要加锁。

*   **巨对象 (> 16MB)**：
    *   Go 运行时对超大对象的处理更为特殊，通常会直接向操作系统申请，并且这部分内存的管理和回收策略也可能与其他对象有所不同。

### 总结

Go 的内存分配过程是一个设计精巧的分层缓存系统：
